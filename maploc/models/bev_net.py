...
class BEVNet(BaseModel):
    ...
    def _forward(self, data):
        features = self.blocks(data["input"]) # [b, 128, 64, 129]
        pred = {
            "output": self.output_layer(features),
        }
        pred["features"] = features # features for cross-attention
...
class BEVNet_Encoder(BaseModel):
    default_conf = {
        "pretrained": True,
        "num_blocks": "???",
        "latent_dim": "???",
...
class BEVNet_Decoder(BaseModel):
    default_conf = {
        "pretrained": True,
        "num_blocks": "???",
        "latent_dim": "???",
