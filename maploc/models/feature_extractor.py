...
class FeatureExtractor(BaseModel):
    ...
    def _forward(self, data):
        ...
        for adapt, i in zip(self.adaptation, self.conf.output_scales):
            # out_features.append(adapt(pre_features[i])) # adaptation: for feature compression in channel dimension
            out_features.insert(0,adapt(pre_features[i])) # fine to coarse
...
class FeatureEncoder(BaseModel):
    ...
    def _forward(self, data):
        ...
        pred = {"feature_maps": features, "skip_features": skip_features}
        return pred
