...
def in_matrix(camera):
    """construct intrinsic matrix of camera"""
    f, c = camera.f, camera.c
    B = camera.f.shape[0]
    intr = torch.zeros((B,3,3))
    intr[:,2,2] = 1.0
    intr[:,0,0] = f[...,0]
    intr[:,1,1] = f[...,1]
    intr[:,0,2] = c[...,0]
...
def in_matrix_inv(camera):
    """construct !!!inverse intrinsic matrix of camera"""
    f ,c = camera.f, camera.c
    f_inv = 1.0 / f
    B = camera.f.shape[0]
    intr_inv = torch.zeros((B,3,3))
    intr_inv[:,2,2] = 1.0
    intr_inv[:,0,0] = f_inv[...,0]
    intr_inv[:,1,1] = f_inv[...,1]
    intr_inv[:,0,2] = -c[...,0] * f_inv[...,0]
    intr_inv[:,1,2] = -c[...,1] * f_inv[...,1]
