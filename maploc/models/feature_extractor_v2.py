...
class Bottleneck_noexp(Bottleneck):
    expansion = 1

class Decoder_Bottleneck(nn.Module):
    def __init__(
        self, previous, out, ksize=3, num_convs=1, norm=nn.BatchNorm2d, padding="zeros",align_corners = False
    ):
        super().__init__()
        self.align_corners = align_corners
        layers = []
        for i in range(num_convs):
            block = Bottleneck_noexp(previous, out, downsample=nn.Conv2d(previous,out,1))
            layers.append(block)
            # if norm is not None:
    ...
    def forward(self, previous, skip):
        ...
        upsampled = nn.functional.interpolate(
            previous, size = skip.shape[2:], mode="bilinear", align_corners=self.align_corners
        )
...
class FPN_Bottleneck(nn.Module):
    def __init__(self, in_channels_list, out_channels, **kw):
        ...
        self.blocks = nn.ModuleList(
            [
                Decoder_Bottleneck(c, out_channels, **kw)
                for c in in_channels_list[::-1][1:]
            ]
        )
        self.out = nn.Sequential(
...
class FeatureEncoder(BaseModel):
    ...
    def _forward(self, x):
        ...
        outputs = self.encoder(x) # [b, 2048, 16, 16]
        return outputs
...
class FeatureExtractor(BaseModel):
    ...
    def _forward(self, data):
        ...
        skip_features = self.encoder(image) # [b, 2048, 16, 16]
